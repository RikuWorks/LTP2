...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Warm-start LWIRPOSE fine-tuning from the OTP-finetuned direct model."
    )
    parser.add_argument("--config", type=str, default="configs/lwirpose_finetune_pose_direct.yaml")
    parser.add_argument(
        "--source-suite",
        type=str,
    ...
    parser.add_argument("--output", type=str, default="outputs/lwirpose_pose_direct_from_otp")
    return parser.parse_args()


def source_paths(source_suite: str) -> tuple[Path, Path]:
    root = Path(source_suite) / LWIRPOSE_MODEL
...
def main() -> None:
    ...
    cfg.model.name = LWIRPOSE_MODEL
    model_root = ensure_dir(Path(args.output) / LWIRPOSE_MODEL)
