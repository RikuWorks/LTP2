...
TARGET_MODEL_NAME = "pose_resnet101_se_thermal_dualpath_ppm"
RUN_NAME = TARGET_MODEL_NAME


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train and benchmark the dualpath PPM model from the standard PPM weights.")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/openthermalpose2_finetune_dualpath_from_ppm.yaml",
    )
    parser.add_argument(
        "--source-suite",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="outputs/dualpath_from_ppm",
    )
    parser.add_argument(
        "--benchmark-output",
        type=str,
        default="outputs/otp2_test_benchmark_dualpath_from_ppm",
    )
...
def main() -> None:
    args = parse_args()
    print("training:", TARGET_MODEL_NAME, "from", SOURCE_MODEL_NAME)
    run_training(args)
    run_benchmark(args)
