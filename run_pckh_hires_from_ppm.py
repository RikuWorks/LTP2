...
BASE_MODEL_NAME = "pose_resnet101_se_thermal_ppm"
RUN_NAME = "pose_resnet101_se_thermal_ppm_pckh_hires"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="High-resolution low-lr short-epoch PCKh refinement from the standard PPM model.")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/openthermalpose2_finetune_pckh_hires_from_ppm.yaml",
        help="Refine config.",
    )
    parser.add_argument(
        "--source-suite",
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="outputs/pckh_hires_from_ppm",
        help="Output directory for the refinement run.",
    )
    parser.add_argument(
        "--benchmark-output",
        type=str,
        default="outputs/otp2_test_benchmark_pckh_hires_from_ppm",
...
def source_paths(source_suite: str) -> tuple[Path, Path]:
    root = Path(source_suite) / BASE_MODEL_NAME
...
def prepare_detector_copy(source_detector_ckpt: Path, run_root: Path) -> None:
...
def run_refine(args: argparse.Namespace) -> Path:
    ...
    run_root = ensure_dir(Path(args.output) / RUN_NAME)
    prepare_detector_copy(source_detector_ckpt, run_root)

    cfg = load_config(args.config)
    cfg.model.name = BASE_MODEL_NAME
...
def run_benchmark(args: argparse.Namespace) -> None:
    ...
    if not runs:
        raise FileNotFoundError("No runnable model checkpoints were found after refinement.")
    for run in runs:
        if run.model_name == RUN_NAME:
            run.model_name = BASE_MODEL_NAME
...
def main() -> None:
    args = parse_args()
    print("refining high-resolution low-lr short-epoch from:", BASE_MODEL_NAME)
    run_refine(args)
    run_benchmark(args)
