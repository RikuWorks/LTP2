...
from lite_therm_pose.trainers import train_pose
from lite_therm_pose.utils import ensure_dir


REFINE_MODEL_NAME = "pose_resnet101_se_thermal_ppm"
REFINE_RUN_NAME = "pose_resnet101_se_thermal_ppm_pckh_refine"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refine the standard PPM model for PCKh using existing finetuned weights.")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/openthermalpose2_finetune_pckh_refine_from_ppm.yaml",
        help="Refine config for pose-only additional finetuning.",
    )
    parser.add_argument(
        "--source-suite",
        type=str,
        default="outputs/resnet_novel_suite",
        help="Suite directory containing the original pose_resnet101_se_thermal_ppm checkpoints.",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="outputs/pckh_refine_from_ppm",
        help="Output directory that will contain finetune_detector and finetune_pose.",
    )
    parser.add_argument("--benchmark-output", type=str, default="outputs/otp2_test_benchmark_pckh_refine_from_ppm")
...
def source_paths(source_suite: str) -> tuple[Path, Path]:
    root = Path(source_suite) / REFINE_MODEL_NAME
...
def prepare_detector_copy(source_detector_ckpt: Path, run_root: Path) -> Path:
    detector_dir = ensure_dir(run_root / "finetune_detector")
    target = detector_dir / "detector_last.pt"
    if not target.exists():
        shutil.copy2(source_detector_ckpt, target)
    return target


def run_refine(args: argparse.Namespace) -> Path:
    source_detector_ckpt, source_pose_ckpt = source_paths(args.source_suite)
    run_root = ensure_dir(Path(args.output) / REFINE_RUN_NAME)
    prepare_detector_copy(source_detector_ckpt, run_root)

    cfg = load_config(args.config)
    cfg.model.name = REFINE_MODEL_NAME
    pose_dir = run_root / "finetune_pose"
    train_pose(cfg, pose_dir, weights=str(source_pose_ckpt))
    return run_root
...
def main() -> None:
    args = parse_args()
    print("refining:", REFINE_MODEL_NAME, "->", REFINE_RUN_NAME)
    run_refine(args)
    run_benchmark(args)
