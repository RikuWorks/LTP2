...
PCKH_TARGET_MODELS = ["pose_resnet101_se_thermal_ppm_xh"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the PCKh-focused training and OTP2 test benchmark in one command.")
    parser.add_argument(
        "--pretrain-config",
        type=str,
        default="configs/coco_pretrain_paper_grade_fast.yaml",
        help="Pretraining config. On Ubuntu you can pass /home/motomochi/LTP2/configs/coco_pretrain_paper_grade_fast.yaml",
    )
    parser.add_argument(
        "--finetune-config",
        type=str,
        default="configs/openthermalpose2_finetune_pckh_focus.yaml",
        help="Fine-tuning config focused on PCKh.",
    )
    parser.add_argument(
        "--train-output",
        type=str,
        default="outputs/pckh_focus_suite",
        help="Output directory for training runs.",
    )
    parser.add_argument(
        "--benchmark-output",
        type=str,
        default="outputs/otp2_test_benchmark_pckh_focus",
        help="Output directory for OTP2 test benchmark results.",
    )
...
def run_benchmark(args: argparse.Namespace) -> None:
    bench_args = argparse.Namespace(
        ...
        models=",".join(PCKH_TARGET_MODELS),
...
def main() -> None:
    args = parse_args()
    print("pckh target models:", ",".join(PCKH_TARGET_MODELS))
    run_suite(args.pretrain_config, args.finetune_config, PCKH_TARGET_MODELS, args.train_output)
    run_benchmark(args)
