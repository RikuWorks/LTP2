...
BEST_MODEL = "pose_resnet101_se_thermal_elite_ppm"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the best new elite model from scratch and benchmark it on OTP2 test.")
    parser.add_argument("--pretrain-config", type=str, default="configs/coco_pretrain_elite.yaml")
    parser.add_argument("--finetune-config", type=str, default="configs/openthermalpose2_finetune_elite.yaml")
    parser.add_argument("--train-output", type=str, default="outputs/elite_model")
    parser.add_argument("--benchmark-output", type=str, default="outputs/otp2_test_benchmark_elite_model")
...
def main() -> None:
    args = parse_args()
    print("best elite model:", BEST_MODEL)
    run_suite(args.pretrain_config, args.finetune_config, [BEST_MODEL], args.train_output)
    run_benchmark(args)
