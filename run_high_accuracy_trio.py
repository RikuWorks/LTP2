...
SOURCE_MODEL_NAME = "pose_resnet101_se_thermal_ppm"
HIGH_ACCURACY_TRIO = [
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train and benchmark three new high-accuracy models from scratch-initialized PPM weights.")
    parser.add_argument("--config", type=str, default="configs/openthermalpose2_finetune_highacc_trio.yaml")
    parser.add_argument("--source-suite", type=str, default="outputs/resnet_novel_suite")
    parser.add_argument("--output", type=str, default="outputs/high_accuracy_trio")
    parser.add_argument("--benchmark-output", type=str, default="outputs/otp2_test_benchmark_high_accuracy_trio")
...
def source_paths(source_suite: str) -> tuple[Path, Path]:
    root = Path(source_suite) / SOURCE_MODEL_NAME
...
def prepare_detector_copy(source_detector_ckpt: Path, model_root: Path) -> None:
    detector_dir = ensure_dir(model_root / "finetune_detector")
...
def train_models(args: argparse.Namespace) -> None:
    ...
    for model_name in HIGH_ACCURACY_TRIO:
        model_root = ensure_dir(Path(args.output) / model_name)
        prepare_detector_copy(source_detector_ckpt, model_root)
        cfg = load_config(args.config)
        cfg.model.name = model_name
        cfg.model.detector_name = SOURCE_MODEL_NAME
        cfg.model.pose_name = model_name
        train_pose(cfg, model_root / "finetune_pose", weights=str(source_pose_ckpt))
...
def run_benchmark(args: argparse.Namespace) -> None:
    bench_args = argparse.Namespace(
        ...
        models=",".join(HIGH_ACCURACY_TRIO),
...
def main() -> None:
    args = parse_args()
    print("high-accuracy trio:", ",".join(HIGH_ACCURACY_TRIO))
    train_models(args)
    run_benchmark(args)
